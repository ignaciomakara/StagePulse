...
The HTTP/WebSocket bridge serves `/stage`, `/audience/{stage_id}`, `/overlay/{stage_id}`, and `/control`. Caption views subscribe through `/ws/stages/{stage_id}/captions`. Control Room polls the real status API; it does not read or produce caption text. Audience QR generation uses an HTTP(S) origin from the request or `STAGEPULSE_PUBLIC_BASE_URL` and runs locally. UI language preference is browser-local and independent of caption language.
