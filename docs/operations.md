...
5. **Share Audience QR.** Confirm the URL shown on Stage Console is reachable by phones. If it says localhost, set `STAGEPULSE_PUBLIC_BASE_URL` to the real LAN or HTTPS origin and restart the server. Share the QR or copy the link. Viewers can choose caption language independently of UI language.
